...
class RelationClassifier:
    ...
    def train(self, train_data, dev_data):
        ...
        report = metrics.get_classification_report(
                            ...
                            dev_rels.tolist(), dev_outputs.argmax(1).tolist(), output_dict=True
                        )
        report_df = pd.DataFrame(report).transpose()
        report_df.to_csv(report_path, sep="\t", index=False)


    def evaluate(self, test_data, model_path=None):
        """Evaluation function for the neural network

        Multiple metrics are computed within this function, including a classification report (per class
        precision, recall, and f1), hits@3, hits@10, and mean reciprocal rank. If the command line parameter to log the
        metrics is passed, all metrics are logged using mlflow.

        Args:
            test_data: test data composed of (embeddings, labels, indices)
        """


        if model_path:
            para = {}
            for p in model_path.split(".pt")[0].split("&"):
                k,v = p.split("=")
                para[k] = v

            self.model = NN(
            emb_dim = int(para["emb_dim"]),
            out_dim = int(para["n_classes"]),
            n_hid1 = int(para["h_dim1"]),
            )
        ...
        with torch.no_grad():
            ...
            test_hits3 = metrics.hits_at_k(test_ranks, k=3)
            test_hits10 = metrics.hits_at_k(test_ranks, k=10)
            test_mrr = metrics.mrr(test_ranks)

            if args.print_output:
                print(test_report)
                print(f"\tAcc: {test_acc:.4f} (test)")
                print(f"\tF1: {test_f1:.4f} (test)")
                print(f"\thitsat3: {test_hits3:.4f} (test)")
                print(f"\thitsat10: {test_hits10:.4f} (test)")
                print(f"\tmrr: {test_mrr:.4f} (test)")

            # Log metrics
            if self.params.log_metrics:
                mlflow.log_metric("Test acc", test_acc)
                mlflow.log_metric("Test f1", test_f1)
                mlflow.log_metric("Test hitsat3", test_hits3)
                mlflow.log_metric("Test hitsat10", test_hits10)
                mlflow.log_metric("Test mrr", test_mrr)

        # Save classification report
        if self.model_path:
            report_path = self.model_path.split(".pt")[0]+".test.report.tsv"
        else:
            report_path = "test.report.tsv"
        # report = metrics.get_classification_report(test_rels.tolist(), test_outputs.argmax(1).tolist(), classes, output_dict=True)
        report = metrics.get_classification_report(test_rels.tolist(), test_outputs.argmax(1).tolist(), output_dict=True)
        report_df = pd.DataFrame(report).transpose()
        report_df.to_csv(report_path, sep="\t", index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=False,
        default=False,
        help="Set to path of model file.",
    )
    parser.add_argument(
        "--log-metrics",
        required=False,
    ...
    for file in config_files:
        ...
        classifier = RelationClassifier(args, config["parameters"])
        ...
        if args.test:
            ...
            if args.model_path:
                classifier.evaluate(test_data, args.model_path)
            else:
...
# ssh imsMachineKiwi
# yes
# cd /mount/studenten/dialog-system/2020/student_directories/Tsereteli_Tornike/adviser
# source venv/bin/activate
# cd adviser/examples/qa/semanticparsing/
# python3 relationpredictor.py --train True --test True --log-metrics True --config 